...
*/	
	...
	var obj = null;
		...
		switch (objtype) {
				...
				case 'Rect':
					activebutton = 'rect';
					obj = newobj(true,{name:'none',id:'none',type:'Rect',x:xpos,y:ypos,width:50,height:100,lockaspect:false,rotation:0,fill:defcolour,stroke:defstroke,strokeWidth:defstrokewidth,cornerRadius:0,visible:true,opacity:1.0});
				
				break;	
				
				case 'Ellipse':
					activebutton = 'ellipse';
					obj = newobj(true,{name:'none',id:'none',type:'Ellipse',x:xpos+30,y:ypos+30,width:50,height:80,lockaspect:false,rotation:0,fill:defcolour,stroke:defstroke,strokeWidth:defstrokewidth,visible:true,opacity:1.0});
				break;									
				
				case 'RegularPolygon':
					activebutton = 'polygon';
					obj = newobj(true,{name:'none',id:'none',type:'RegularPolygon',x:xpos+30,y:ypos+30,rotation:0,sides:5,radius:30,fill:defcolour,stroke:defstroke,strokeWidth:defstrokewidth,visible:true,opacity:1.0});
				break;									

				case 'Star':
					activebutton = 'star';
					obj = newobj(true,{name:'none',id:'none',type:'Star',x:xpos+30,y:ypos+30,rotation:0,numPoints:5,innerRadius:15,outerRadius:30,fill:defcolour,stroke:defstroke,strokeWidth:defstrokewidth,visible:true,opacity:1.0});
				break;									

				case 'Ring':
					activebutton = 'ring';
					obj = newobj(true,{name:'none',id:'none',type:'Ring',x:xpos+30,y:ypos+30,innerRadius:15,outerRadius:30,fill:defcolour,stroke:defstroke,strokeWidth:defstrokewidth,visible:true,opacity:1.0});
				break;	
				
				case 'Text':
					activebutton = 'text';
					obj = newobj(true,{name:'none',id:'none',type:'Text',x:xpos+30,y:ypos+30,rotation:0,text:'text',fontSize:24,fontFamily:'Arial',fill:defcolour,stroke:defstroke,strokeWidth:1,visible:true,opacity:1.0});
				break;	

				case 'Line':
					activebutton = 'line';
					obj = newobj(true,{name:'none',id:'none',type:'Line',x:xpos+30,y:ypos+30,lineLength:100,rotation:0,stroke:defstroke,strokeWidth:defstrokewidth,visible:true,opacity:1.0});
				break;	
				
				case 'PolyLine':
					activebutton = 'polyline';
					obj = newobj(true,{name:'none',id:'none',type:'PolyLine',x:xpos+30,y:ypos+30,fill:defcolour,rotation:0,stroke:defstroke,strokeWidth:defstrokewidth,vertices:3,tension:0,points:[0,0,-30,100,100,100],closed:true,scaleSize:1.0,visible:true,opacity:1.0});
				break;					
				
				case 'CurvedArrow':
					activebutton = 'curvedarrow';
					obj = newobj(true,{name:'none',id:'none',type:'CurvedArrow',x:xpos+30,y:ypos+30,rotation:0,fill:defcolour,stroke:defstroke,strokeWidth:defstrokewidth,visible:true,opacity:1.0,portion:1.0,midX:100,midY:50,endX:100,endY:100,arrowWidth:ar_width,arrowheadLength:ar_headlength,arrowheadWidth:ar_headwidth});
				break;	

				case 'Mfigure':
					activebutton = 'mfig';
					obj = newobj(true,{name:'none',id:'none',type:'Figure',x:xpos+30,y:ypos+30,fill:defcolour,rotation:0,stroke:defstroke,strokeWidth:defstrokewidth,tension:0,points:[0,0,-3.5,1,-7,5,-9,10,-7,16,-2,19,-3,22,-9,22,-22,54, -21,55,-20,57,-16,57,-10,40,-10,90,-8,92,-5,93,-3,90,-2,60,2,60,3,90,5,93,8,92,10,90,10,40,16,57,20,57, 21,55,22,54,9,22,3,22,2,19,7,16,9,10,7,5,3.5,1],scaleSize:1.0,visible:true,opacity:1.0});

				break;					
				
				case 'Ffigure':
					activebutton = 'ffig';
					obj = newobj(true,{name:'none',id:'none',type:'Figure',x:xpos+30,y:ypos+30,fill:defcolour,rotation:0,stroke:defstroke,strokeWidth:defstrokewidth,tension:0,points:[0,0,-3.5,1,-7.5,5,-9,10,-6.7,16,-2,19,-3,22,-9,22,-23,54, -21,55,-19,57,-16,56,-10,40,-15,70,-10,70,-10,90,-8,93,-4.5,93,-3,90,-3,70,3,70,3,90, 4.5,93,8,93,10,90,10,70,15,70,10,40,16,56,19,57, 21,55,23,54,9,22,3,22,2,19,6.7,16,9,10,7.5,5,3.5,1,0,0],scaleSize:1.0,visible:true,opacity:1.0});
				break;					
			}								
...
function newPresentEvent()
{
	...
	var peviewarr = [];
	var pestate = {name:'event',index:ind,id:UniqueId(),actiontype:'Eventlist'};
...
function newView() 
{
	...
	if (views.length < maxviews) {
		...
		for (var pei=0;pei<presentevents.length;pei++) {
			...
			peviewstate.peviews.push({id:UniqueId(),viewstate:viewstate,layerid:'none',layername:'none',actions:[]});
		}
		txPEventsArr();
...
function castPresentEvent()
{
	if (activepeevobj != null) {
		...
		if (castind>-1) {
			var msg = JSON.stringify({command:'castPEinfo',info:castind});
...
function txPEventsDelete(index)
{
	...
	var msg = JSON.stringify({command:'deleteEventList',info:index});
	socket.emit('updateEvents', msg);
}
...
function txPEventsCheckDesign()
{
	...
	var msg = JSON.stringify({command:'checkDesignScreen',info:''});
	socket.emit('updateEvents', msg);
}
...
function txPEventsGetLayerInfo()
{
	...
	var msg = JSON.stringify({command:'getLayerinfo',info:''});
	socket.emit('updateEvents', msg);
}
...
function txPEventsGetPEInfo()
{
	...
	var msg = JSON.stringify({command:'getPEinfo',info:''});
	socket.emit('updateEvents', msg);
}
...
function txplayPEEvents(index)
{
	...
	var msg = JSON.stringify({command:'playPE',info:index});
	socket.emit('updateEvents', msg);
}
...
function txstartPEEvents(index)
{
	...
	var msg = JSON.stringify({command:'startPE',info:index});
	socket.emit('updateEvents', msg);
}

function txplayEvent(peobj)
{
	//message to designscreen to play
...
{
	//message to designscreen to go to startstate
	var ind = presentevents.indexOf(peobj);
	txstartPEEvents(ind);	
...
presstage = new Kinetic.Stage({
	...
	preslayer = new Kinetic.Layer({name:"preslayer"});
